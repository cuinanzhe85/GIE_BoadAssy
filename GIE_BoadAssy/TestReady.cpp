...
void CTestReady::OnTimer(UINT_PTR nIDEvent)
{
	...
	if(nIDEvent == 1)
	{
		...
		if(m_pApp->m_pDio7250->Gf_getDIOJigArrive())
		{
			GetDlgItem(IDC_STT_STATUS_MSG)->SetWindowText(_T("JIG ARRIVED"));

			if(m_pApp->m_pDio7250->Gf_getDIOTestStart())
			{
	...
	else if(nIDEvent == TIMER_PID_CHECK)
	{
		...
		{			
			m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_BCR_READ_OK);
			m_pApp->Gf_writeLogData(_T("[PC -> PLC]"), _T("BCR READ OK"));
...
BOOL CTestReady::PreTranslateMessage(MSG* pMsg)
{
	...
	if (pMsg->message == WM_KEYDOWN)
	{
		...
		case VK_F12:
			{
				m_pApp->Gf_writeLogData(_T("KEY"), _T("F12"));
			}return TRUE;
...
void CTestReady::Lf_initFontSet()
{
	...
	GetDlgItem(IDC_STT_TOTAL_CNT_VALUE)->SetFont(&m_Font[0]);
	GetDlgItem(IDC_STT_GOOD_CNT_VALUE)->SetFont(&m_Font[0]);
	GetDlgItem(IDC_STT_BAD_CNT_VALUE)->SetFont(&m_Font[0]);
	...
	m_Brush[3].CreateSolidBrush (RGB(86,86,86));
...
bool CTestReady::Lf_getControlBdReady()
{
	...
	m_pApp->Gf_writeLogData(_T("CTRL B/D Ready Check"),_T("START"));
	...
	m_pApp->Gf_writeLogData(_T("CTRL B/D Ready Check"), _T("END"));
...
void CTestReady::Lf_checkPanelId()
{
	...
	m_pApp->Gf_writeLogData(_T("[PANEL ID COPY] SEEK BCR DATA"), lpWorkInfo->m_sReceivePID);

	lpWorkInfo->m_sPID = lpWorkInfo->m_sReceivePID;
	
	if(lpWorkInfo->m_sPID.GetLength() <= 9)
	{
		m_pApp->Gf_writeLogData(_T("[PANEL ID LENGTH ERROR] LESS 9"), lpWorkInfo->m_sPID);
		CPanelID pidDlg;
		pidDlg.DoModal();
	}
	else
	{		
		...
		{ 
			m_pApp->Gf_writeLogData(_T("[PANEL ID DATA ERROR] %s "), lpWorkInfo->m_sPID);
	...
	m_pApp->Gf_writeLogData(_T("[GMES SET PANEL ID]"), lpWorkInfo->m_sPID);
}
...
BOOL CTestReady::Lf_sendGMESData()
{
	...
	m_pApp->Gf_writeLogData(_T("[GMES]"), _T("SEND DATA"));
	...
	else
	{
		...
		m_pApp->Gf_writeLogData(_T("[GMES BAD CODE]"), _T("DIALOG OPEN"));
		m_pApp->Gf_writeLogData(_T("[GMES BAD CODE OPEN]"), lpWorkInfo->m_sBadCode);
		...
		{
			readDio = (BYTE)m_pApp->m_pDio7250->Dio_DI_ReadPort();
			...
			m_pApp->Gf_writeLogData(_T("[GMES BAD CODE CLOSE]"), outLog.GetBuffer(0));
			...
			m_pApp->Gf_writeLogData(_T("[BAD CODE CLOSE]"), outLog);
...
//imsi				pUiPorc->comm.curData.iPatternEndESC = 0;
				lpWorkInfo->m_nPassOrFail = GMES_PNF_FAIL;
				m_pApp->Gf_writeLogData(_T("[JUDGE NG]"), _T("DO NOT SEND EICR"));

				//TEST OK 신호를 출력하여 JIG가 대기 위치로 이동 하도록 한다. 
				//이후 OK, NG 신호를 PLC로 전달하여 OUT 이제기가 배출 하도록 한다.
				m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_TEST_OK);
				return TRUE;
			}
			 ...
			 || (lpWorkInfo->m_nPassOrFail == GMES_PNF_PASS))
			{
				...
				{
					m_pApp->Gf_writeLogData(_T("[GMES]"), _T("SET B GRADE"));
		...
		else
		{
			...
			outLog.Format(_T("[GMES BAD CODE] OK - %d, NG - %d, B - %d, PnF - %d, BAD DLG - %d, DIO - %d"), 
				...
				defecetResult, 
				readDio);
			m_pApp->Gf_writeLogData(_T("[GMES BAD CODE]"), outLog);

			m_pApp->Gf_writeLogData(_T("[GMES]"), _T("CANCEL"));
			return FALSE;
		}
	}
	...
	m_pApp->Gf_writeLogData(_T("[DIO PC -> PC]"), _T("TEST OK - JIG OUT"));
	m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_TEST_OK);//(DIO_GUMI_M5_TEST_OK, 1);
...
{
	m_pApp->Gf_writeLogData(_T("[JUDGE]"), _T("START"));
	...
	if(lpSystemInfo->m_nOperationMode == IN_LINE) 
	{
		...
		{
			int boxResult = 0;
			m_pApp->Gf_writeLogData(_T("[JUDGE]"), _T("[TEST MODE] TECH or GIENG"));
			...
			if(lpWorkInfo->m_bGieJudgeNg == true || lpWorkInfo->m_nFastJudge == FAST_JUDGE_NG)
			{
				//TEST OK 신호를 출력하여 JIG가 대기 위치로 이동 하도록 한다. 
				//이후 OK, NG 신호를 PLC로 전달하여 OUT 이제기가 배출 하도록 한다.				
				m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_TEST_OK);//Dio_OutPut(DIO_GUMI_M5_TEST_OK, 1);

				Sleep(500); //PLC로 TEST OK, NG 신호를 동시에 들어가도 상관은 없으나 일단 안정적으로 FLOW를 가기 위해 Delay 줌. T/T 과 상관없음

				m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_JUDGE_NG);//Dio_OutPut(DIO_JUDGE_NG, 1);
				m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("JUDGE NG"));
			...
			else if(lpWorkInfo->m_bGieJudgeOk == true || lpWorkInfo->m_nFastJudge == FAST_JUDGE_OK)
			{
				...
				m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_JUDGE_OK);//Dio_OutPut(DIO_JUDGE_OK, 1);	
				m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("JUDGE OK"));
				Lf_createCount(GOOD_CNT);
			}
			else
			{
				m_pApp->m_pDio7250->Gf_setDioWrite(DIO_INIT);//Dio_OutPut(DIO_MAX, 1);
				m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("NO SIGNAL"));
			}
		}
		else
		{
			if(Lf_sendGMESData()==TRUE)
			{
				if(lpWorkInfo->m_nPassOrFail == GMES_PNF_PASS) 
				{
					...
					m_pApp->m_pDio7250->Gf_setDioWrite(DIO_JUDGE_OK);//Dio_OutPut(DIO_JUDGE_OK, 1);
					m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("JUDGE OK"));
				...
				else if(lpWorkInfo->m_nPassOrFail == GMES_PNF_FAIL)
				{
					...
					m_pApp->m_pDio7250->Gf_setDioWrite(DIO_JUDGE_NG);//Dio_OutPut(DIO_JUDGE_NG, 1);
					m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("JUDGE NG"));
				}
			}
		}
...
void CTestReady::Lf_updateCount()
{
	...
	m_pApp->Gf_writeLogData(_T("[CREATE COUNT] "), sdata);
}
...
bool CTestReady::Lf_startTest()
{
	...
	Sleep(lpSystemInfo->m_nTestStartDelay);
	...
	{
		Sleep(lpSystemInfo->m_nTestStartDelay);
		m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_PRESS_NOMAL_UP);
		m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("PRESS UP"));
		if(lpWorkInfo->m_bIsEdidFail == true)
		{
			m_pApp->m_pDio7250->Gf_setDioWrite(DIO_GUMI_M5_EDID_NG);
			m_pApp->Gf_writeLogData(_T("[DIO PC -> PLC]"), _T("EDID NG"));
		}
	}
